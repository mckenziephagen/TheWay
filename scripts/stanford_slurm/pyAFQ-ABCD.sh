...
datalad get -n "inputs/data/${subid}_qsiprep-v0.14.2.zip"
echo datalad got
...
datalad run \
    -i code/pyafq_zip.sh \
    -i inputs/data/${subid}_qsiprep-v0.14.2.zip \
    -i containers/.datalad/environments/pyafq-0-12-2/image \
    --expand inputs \
    --explicit \
    -o ${subid}_${sesid}_pyafq-0.12.2.zip \
...
7z x "inputs/data/${subid}_qsiprep-v0.14.2.zip" -oderivatives
...
rm -f derivatives/afq/tract_profiles.csv

#zip results
7z a ${subid}_${sesid}_pyafq-0.12.2.zip derivatives/afq
